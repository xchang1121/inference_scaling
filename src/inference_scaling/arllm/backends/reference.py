...
class ReferencePolicyBackend:
    ...
    def sample_batch(self, requests):
        inner_requests = [
            replace(request, sampling=self._policy(request.sampling),
                    reference_temperature=request.reference_temperature * self.temperature)
        ...
        for request, inner_request, sample in zip(requests, inner_requests, samples, strict=True):
            ...
            reference = inner_request.reference_policy.policy_id
            if sample.policy_id == reference:
                reference_logs = sample.token_logprobs
            elif sample.reference_policy_id == reference:
            ...
            result.append(replace(
                ...
                reference_policy_id=None if reference_logs is None else request.reference_policy.policy_id,
            ))
        return result
