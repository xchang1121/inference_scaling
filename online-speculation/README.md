...
从已有自回归基座出发，独立实现条件低秩适配器、离线块去噪蒸馏、精确验证解码和推理后续训。
在线路径覆盖原扩散适配器的续训，以及 PrefixRelay 条件起草头的学习。
算法来源以 [Uno 原论文](https://arxiv.org/abs/2609.04010) 为起点，模型、训练器和推理流程实现在 `src/blockspec`。
