...
在固定输入和足够表达能力下，教师质量高于学生的位置获得提高分数的梯度，反之降低。离线入口同时保存优化器、随机数、数据顺序和学习率调度，使中断恢复对应连续训练中的同一更新序列。
...
设每 $s$ 个有效轮次更新一次，每次重放最近 $r$ 个反馈块，其中 $1\le r\le s$。窗口中的损失按有效位置数归一化：

$$
\mathcal L_t(\phi)=
\frac{\sum_{b\in\mathcal R_t}\sum_{i=1}^{n_b}
D_{\mathrm{KL}}(p_{b,i}\Vert q_{\phi,b,i})}
...
当前续训支持完整分布 KL 和 TV。KL 具有平滑的 logits 梯度，TV 直接对应相同概率规则下的平均接受率。

两者之间有一个有用的定量关系。令 $E=\{v:p(v)\ge q(v)\}$，则
...
此界要求左右两侧使用同一对分布。当前完整分布训练以温度一的原始 softmax 为对象；部署中的温度和截断定义了另一对 $\mathcal S(u^{\mathrm{AR}}),\mathcal S(u^D)$。因此评估同时记录原始 KL、原始 TV 和实际采样 TV。后续扩展聚焦实际采样分布的重叠，并以独立请求流上的净吞吐选择方案。

### 6.5 学习界的适用条件

对固定反馈分布，设平均损失 $F(\phi)$ 下界为 $F_\ast$，梯度满足 $L$-光滑条件。若梯度估计 $g_t$ 无偏、方差至多 $\sigma^2$，并使用
...
同一问题交错运行各组，保持模型精度、提示模板、采样规则、EOS 规则和输出预算一致。每组 TPS 为输出 token 总数除以请求总时间。置信区间以问题为簇配对重采样；多条独立学习流用于检验在线过程对问题顺序和随机性的敏感程度。
