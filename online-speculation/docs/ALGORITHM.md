...
基础阶段在 WSL2 / PyTorch 2.11.0+cu128 上 **123 项测试通过**，含小型 Qwen3 数值参照、树路径、
...
不按 EOS 截断；正反顺序各一轮，每个方法合计 1,024 个输出 token。B=4，线性路径，
在线每 32 轮用最近 1 个反馈块做 L1 更新，学习率 $10^{-4}$。AR 也使用直接 argmax 的贪心优化。

| 方法 | 总生成秒数 | TPS | 相对同场 AR | 平均每轮输出 |
| --- | --- | --- | --- | --- |
| 普通自回归 | 15.650 | 65.43 | 1.000× | 1.000 |
| 固定离线适配器 | 14.153 | 72.35 | 1.106× | 2.977 |
| 在线全适配器续训 | 14.635 | 69.97 | 1.069× | 2.960 |

在线 10 次实际更新累计 0.690 秒；另外计入两次 learner 初始化约 0.0023 秒后仍为 69.96 TPS。
所有对照贪心输出一致，基座不变，每个在线流的适配器均发生真实变化。峰值已分配显存约 4.63 GiB。
离线检查点 SHA 为 `4dc0e670082b097f4149a930ba211012376160b03f20e85d367454a707882472`，
验证文件 SHA 为 `27de07376f8ff0c100782da6e5e69defac9ae5ce353a806209da16e3a8f72eb2`；入口见运行说明。

结论限于：这个小规模请求流中，学习开销尚未吃掉离线投机的全部加速；
**没有证明在线学习提高了接受长度或超过静态版**，表中在线连续接受反而略低。
...
4. 同条件三基线：独立评测入口及首轮真实线性结果已完成，继续扩大开发覆盖，再一次性使用保留测试。
5. 在线净收益：按请求顺序测累计 tokens／累计秒数，包含学习成本，检查主题迁移。
6. 树路径：概率、兄弟隔离、缓存压缩、反馈对齐及真实权重 FP32 小检查已通过；继续测收敛适配器的净 TPS。
