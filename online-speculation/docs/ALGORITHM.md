...
它对应[原论文式 (3)](https://arxiv.org/pdf/2609.04010) 的联合热身设置，而不是纯反向 KL，
也不是反向 KL 加数学定义中除以 2 的 TV。热身结束后去掉 KL 项，保留 L1。
对相同输入，测试比较联合目标的数值与每项梯度之和，并确认老师没有梯度。
...
仍可显式选择纯 KL、前向 KL 热身或单一目标，这些选项不混称同一配方。
热身计入总步数；块长按训练进度依次切换，不是每步随机抽块长。数据窗口与噪声各用独立随机数生成器。
...
$q_j[\log(q_j/p_j)-R+s_j-\sum_iq_i s_i]$，绝对值项对应未除以 2 的 L1。
这里也带 $q_j$ 因子，所以“联合热身来自原配方”并不逻辑上排除前向 KL 在某个小数据启动实验中更有效。
...
当前路径是在已验证的节点从目标 p 抽下一项：树里有对应子节点就继续，没有就发出该 token 并结束。
...
则先从目标 logits 抽 token，再调用树遍历。此前把这条实现当成“另一个尚未对齐的随机拒绝算法”不准确，现据源码契约纠正。
...
目标的 top-k/top-p 截断不用于树评分，树有自己的候选 top-k。分数只决定计算哪些节点，不用于接受率分母。
...
`scripts/audit_sampler_reference.py` 是可选的只读参照检查，不属于训练／服务依赖。
它从固定 Git 对象读取 CPU 树构建与遍历定义，避开作者模型、引擎、包初始化和 GPU 内核；本项目实现没有复制这些函数。
16 个无并列分数的树构建配置、1,600 组预先抽定的目标 token 路径，节点、父亲、深度、提交序列及缓存路径均匹配。
使用的 commit 和源文件 SHA 会随检查打印，原始结果不落盘。

这个对照检查的是输入／输出契约，不能推出 GPU 内核、同分数时的排序或 TPS 一致；
本项目同分数时按 token 路径打破平局，参照实现使用深度、候选排名等次级键。
另外的小词表测试穷举了两步目标联合分布，覆盖沿树继续、走出树再恢复 AR 的情况，避免只验证第一项边缘分布。
...
基础阶段在 WSL2 / PyTorch 2.11.0+cu128 上 **139 项测试通过**，含小型 Qwen3 数值参照、树路径、
...
前 150 步使用反向 KL＋未除以 2 的 L1，之后纯 L1，块长按 2→4 推进、各占 300 步，学习率 $3\times10^{-4}$，全程 FP32。
适配器有 27,983,872 个参数，未加载发布的适配器。训练连同固定验证耗时 134.91 秒，基座指纹不变。
联合热身的目标对应原文式 (3)，但这里的数据量、秩、步数和课程是本机有界实验设置，不冒称复现了全部训练预算。
...
不按 EOS 截断；32-token 预热，正反顺序各一轮，每个方法合计 4,096 个输出 token。B=4，目标路径树，
每位置 top-k=4、前缀预算 12（含根）。在线每 32 轮用最近 1 个反馈块做 L1 更新，学习率 $10^{-4}$，
本表采用最后 4 层特征重放。AR 也使用直接 argmax 的贪心优化。

| 方法 | 总生成秒数 | TPS | 相对同场 AR | 平均每轮输出 |
| --- | --- | --- | --- | --- |
| 普通自回归 | 66.067 | 62.00 | 1.000× | 1.000 |
| 固定离线适配器 | 52.843 | 77.51 | 1.250× | 3.346 |
| 在线末 4 层续训 | 54.174 | 75.61 | 1.220× | 3.346 |

在线 38 次实际更新累计 0.461 秒；另外计入两次 learner 初始化约 0.0040 秒后为 75.60 TPS。
所有对照贪心输出一致，基座不变，每个在线流的末层适配器均发生真实变化。峰值已分配显存约 4.50 GiB。
...
实现指纹为 `0acae5b370233766dade2a7a77e3cb702e857af241ec9305a64821cb382724eb`，入口见运行说明。
同配置、同检查点另测全适配器续训：同场 AR 为 61.28 TPS、静态为 77.52 TPS、在线为 72.47 TPS（1.183× AR）。
其 37 次更新累计 2.533 秒，平均约 68.5 毫秒；末层版平均约 12.1 毫秒。两次测量不能混用 AR 分母，
也不能把约 5.6 倍的更新区域差异说成整体 TPS 提高 5.6 倍。
全适配器版本同样保持贪心输出与基座不变，参数发生真实更新，峰值已分配显存约 4.85 GiB。
线性解码仍保留为较简单的独立对照入口。
...
6. 树路径：概率、兄弟隔离、缓存压缩、反馈对齐及真实权重检查已通过；与固定公开版本的 CPU 输入／输出契约也完成独立对照。
...
前向 KL 热身也完成过同规模训练与开发检查，但未显示明确优于联合热身的净 TPS；
当前保留联合热身检查点作为复刻主线，前向 KL 仍是代码中的可选配置，不保留该次旧试验权重。

尚未完成大规模蒸馏、批量服务、异步训练、原研究级别的 GPU 执行优化及论文全套基准。
