...
增加 `--reuse-speculative-answers` 后，后续版本继续复用实际交付回答进行完整筛选。
候选与服务版本在整组检查中保持固定；通过筛选后，另一个获准请求额外生成候选与 AR，
分别比较候选／当前服务和候选／AR，两项达到发布余量后更新服务版本。
检查点同时保存服务版本号和用于估算确认成本的实测 AR 吞吐。
首次 AR 服务、训练目标和时间预算沿用完整回答策略。
性能记录第 9 节沿用上述命令，并设置
