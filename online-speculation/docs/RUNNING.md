...
`--prompt-offset 8 --requests 16` 选取提示文件中的第 9–24 条有效记录，索引从零开始。
可同时增加 `--tokens 512`，在新的请求窗口和更长输出上复查相同更新配置。
`--audit-requests 8 --audit-tokens 128` 在吞吐测量完成后增加同前缀审计：
沿原始固定起草的轨迹，分别用原始末层和学习后末层重放同一边界与教师概率。
输出 `common_prefix_audit` 保存原始 softmax 的 KL／TV、实际采样规则下的 TV、逐深度观测数及配对问题区间。
`learned_minus_original` 为学习后减原始的距离差；采样 TV 的负差对应同组前缀上的条件平均接受率提高。
审计过程的生成时间与额外重放单列，吞吐峰值显存于审计启动前记录。
审计保留起草时整块输出投影的形状，再取实际到达的行，并逐轮核对原始重放 logits 与保存的起草 logits。
`--audit-only` 单独执行相同预学习与审计，适用于独立核验分布变化；输出中的吞吐对照为空。
