...
class SuffixLookup:
    """Incremental longest-suffix index with two observed continuation tokens."""

    def __init__(self, tokens=(), *, minimum=2, maximum=8):
...
class ContinuationMix:
    ...
    def __init__(self, block_size, top_k, *, learning_rate=.5, interval=8, adaptive=True,
        ...
        self.memory = SuffixLookup()
        self.group, self.copied = 0, []

    def begin_request(self, tokens):
        self.memory = SuffixLookup(tokens)
        self.copied = []
    ...
    def metrics(self):
        ...
        return {"kind": self.kind, "weights": self.weights.cpu().tolist(),
