...
本节考虑严格的黑盒设置：同一个模型可以被多次调用，但接口只返回文本；模型 token 概率、隐藏状态、外部
verifier 和参考值均不可用。此时式 (1) 相对于 proposal $`q`$ 的重要性权重为

```math
w(y)\propto
\frac{p(y\mid x)\exp\{r(x,y)/\tau\}}{q(y\mid x)}.
```

当候选直接来自基础模型，即 $`q=p`$ 时，概率比消去，仍需计算 $`\exp\{r/\tau\}`$。当
$`r(x,y)=\log p(y\mid x)`$ 时，奖励本身又需要序列概率。因此，纯文本接口无法实现未知奖励目标、
$`p^\alpha`$ 目标或不同 proposal 到基础模型目标的精确重要性校正。

这个限制来自可识别性，而非估计样本不足。设两个任务对所有模型调用给出完全相同的文本分布；第一个任务把
生成 $`y_1`$ 视为正确，第二个任务把 $`y_2`$ 视为正确。任何只读取这些生成文本的算法在两个任务中的输出
分布完全相同，因而无法保证同时提高两个任务的正确率。额外生成可以估计模型的重复性和自我判断，无法恢复
未观测的真实奖励。

“无外部 verifier”与“无评分信号”需要分别处理。同一模型可以在新的提示下输出检查结论、两两比较结果或
文字化置信度；这些文本可转换为可观测的自评权重。所得目标由模型的自评行为定义，其效果取决于自评与任务
正确性的相关性。

### 15.2 独立自检权重的 SIR

令候选 $`Y\sim p(\cdot\mid x)`$。把 $`x,Y`$ 放入固定的检查提示，并用独立随机种子再次调用同一个模型，
得到检查文本 $`\xi`$。确定性解析器把它转换成非负有限权重 $`W(x,Y,\xi)`$。无需读取任何模型概率即可定义
以下边缘目标：

```math
\pi_W(y\mid x)
=
\frac{p(y\mid x)\,\mathbb E[W(x,y,\xi)\mid x,y]}
     {\sum_{y'}p(y'\mid x)\,\mathbb E[W(x,y',\xi)\mid x,y']}.
```

从模型生成 $`N`$ 个候选，对每个候选生成独立检查文本，然后按 $`W_i/\sum_jW_j`$ 重采样。这是定义在
“候选文本、检查文本”联合空间上的 Sampling-Importance-Resampling（SIR，采样—重要性加权—重采样）；
proposal 正好是两次黑盒模型调用的联合分布，因而权重中不需要概率比。对任意有界统计量 $`f`$，只要
$`0<\mathbb E[W]<\infty`$，大数定律直接给出

```math
\frac{\sum_{i=1}^{N}W_i f(Y_i)}{\sum_{i=1}^{N}W_i}
\xrightarrow[N\to\infty]{\mathrm{a.s.}}
\frac{\mathbb E[Wf(Y)]}{\mathbb E[W]}
=\sum_y f(y)\pi_W(y\mid x).
```

因此，增加候选数收敛到上式的自评目标。该结论没有把自评正确率假设为真实正确率。

二元检查便于控制估计噪声。令第 $`k`$ 次检查的解析结果为 $`V_k\in\{0,1\}`$，使用

```math
W=\delta+\frac{1}{K}\sum_{k=1}^{K}V_k,
\qquad \delta>0.
```

若各次检查在给定候选后条件独立，且 $`v(x,y)=\Pr(V_k=1\mid x,y)`$，则

```math
\mathbb E[W\mid x,y]=\delta+v(x,y),
\qquad
\operatorname{Var}(W\mid x,y)=\frac{v(x,y)(1-v(x,y))}{K}.
```

增加检查次数 $`K`$ 只降低权重噪声，不改变 $`\pi_W`$；$`\delta`$ 保证有限候选全部检查失败时仍可
归一化。若使用 $`\exp\{\beta S\}`$ 变换随机文字分数 $`S`$，目标中出现的是
$`\mathbb E[\exp\{\beta S\}\mid x,y]`$，不能改写成
$`\exp\{\beta\mathbb E[S\mid x,y]\}`$。

### 15.3 可用的黑盒信号

| 信号 | 黑盒实现 | 与当前 IS 的关系 | 主要依据 |
| --- | --- | --- | --- |
| 最终结论的样本频率 | 规范化最终结论并统计频数 | 等价类相等度作为成对效用；适用于短答案 | [Self-Consistency](https://openreview.net/forum?id=1PL1NIMMrw)、[Adaptive-Consistency](https://aclanthology.org/2023.emnlp-main.761/) |
| 自由文本的一致性 | 将全部候选交给同一模型，选择最一致的候选 | 可先产生离散选择，也可把多次选择频率作为 $`W`$ | [Universal Self-Consistency](https://openreview.net/forum?id=LjsjHF7nAN)、[Self-Agreement](https://aclanthology.org/2024.findings-acl.230/) |
| 独立样本支持或反驳 | 用其他独立生成检查候选中的陈述 | 支持率直接作为二元随机权重 | [SelfCheckGPT](https://aclanthology.org/2023.emnlp-main.557/) |
| 分解后的独立检查 | 先生成检查问题，在不展示候选结论的上下文中分别求解，再比较检查结果 | 降低检查过程复述候选错误的概率；检查通过率作为 $`W`$ | [Chain-of-Verification](https://aclanthology.org/2024.findings-acl.212/) |
| 候选间直接比较 | 同一模型比较两个候选；交换呈现顺序并重复调用 | 胜率或无矛盾率作为 $`W`$；适合正确答案频率较低的题目 | [Sample, Scrutinize and Scale](https://arxiv.org/abs/2502.01839) |
| 文字化置信度 | 要求模型输出二元判断或 0--100 分，再用确定性解析器读取 | 可作轻量权重；需在同一问题的候选间检验区分能力 | [黑盒置信度评估](https://arxiv.org/abs/2306.13063)、[Confidence-Informed Self-Consistency](https://aclanthology.org/2025.findings-acl.1030/) |
| 成对效用的期望 | 将候选与独立参考样本比较语义或结论一致性 | $`W_i`$ 取平均成对效用，得到采样近似的 Minimum Bayes Risk（MBR，最小贝叶斯风险）选择 | [采样近似 MBR](https://aclanthology.org/2022.emnlp-main.754/) |
| 批判与修订 | 同一模型生成批判，再据此生成新候选 | 扩展候选集合；新候选仍需上述独立评分 | [Self-Refine](https://proceedings.neurips.cc/paper/2023/hash/91edff07232fb1b55a505a9e9f6c0ff3-Abstract-Conference.html) |
| 分支搜索或多实例讨论 | 同一模型承担生成、状态评价或相互质疑 | 扩展低概率候选；调用量较高 | [Tree of Thoughts](https://proceedings.neurips.cc/paper/2023/hash/271db9922b8d1f4dd7aaef84ed5ac703-Abstract.html)、[Multiagent Debate](https://proceedings.mlr.press/v235/du24e.html) |
| 动态生成预算 | 根据已见结论的分散程度向困难提示增加样本 | 提高固定总预算下的候选覆盖率；不提供新的正确性信息 | [Reasoning-Aware Self-Consistency](https://aclanthology.org/2025.naacl-long.184/)、[DynScaling](https://arxiv.org/abs/2506.16043) |

这些方法的经验结论并不一致。大规模候选间比较和重复自检可在部分推理任务上继续改善已经饱和的多数投票；
固定总计算量的研究同时表明，常见预算下增加候选通常比对少量候选执行大量验证更有效。后者在所测设置中需要
约 8 倍计算量才首次追平 Self-Consistency，并建议随总预算增长时更快地增加候选数
（[When To Solve, When To Verify](https://arxiv.org/abs/2504.01005)）。无外部反馈的直接自我纠错还可能把
原本正确的推理改错（[Large Language Models Cannot Self-Correct Reasoning Yet](https://openreview.net/forum?id=IkmD3fKBPQ)）。
因此，候选生成、独立检查和修改后重新评分应分别记录，结果需要在固定总输出 token 或 FLOPs 下比较。

### 15.4 建议的实验算法

建议首先实验“同模型候选 + 独立检查权重”的 SIR，并保持每个组成部分可消融：

1. 固定模型版本、生成提示和采样参数，从同一分布生成 $`N`$ 个候选。候选池与用于估计一致性的参考池分开。
2. 规范化最终结论。对候选与独立参考样本计算结论一致性或任务适用的语义效用，得到 $`A_i\in[0,1]`$。
3. 对每个候选执行 $`K`$ 次独立检查。检查提示先把推理分成可验证的子问题，再在不包含候选结论的新上下文中
   解这些子问题；确定性解析器只接受固定格式的通过或失败结果，得到 $`V_i\in[0,1]`$。
4. 可选地请求 0--100 的文字化置信度并缩放为 $`C_i\in[0,1]`$。在开发集上只检验同一问题内高分候选是否
   更常正确；跨问题的绝对校准不决定重加权效果。
5. 预先固定非负系数，使用

   ```math
   W_i=\delta+\lambda_A A_i+\lambda_V V_i+\lambda_C C_i.
   ```

   按 $`W_i/\sum_jW_j`$ 抽取最终生成时，对应第 15.2 节的 SIR 目标；选择最大权重候选时，对应
   Best-of-$`N`$ 决策规则，不再声称从 $`\pi_W`$ 采样。
6. 预算自适应采用初始估计与最终估计分离。初始样本只决定各提示的候选数和检查次数；最终 $`W_i`$ 使用独立
   新检查计算。每个候选至少保留一次最终检查。依据初始结果删去候选属于近似剪枝，需要单独报告删去比例和
   准确率变化。

推荐的第一组消融为：随机单样本、固定预算 Self-Consistency、文字置信度加权、一致性 MBR、独立分解检查、
三种信号组合，以及相同总输出 token 下的动态预算组合。每种设置同时报告 pass@1、候选覆盖率、检查选择准确率、
候选 token、检查 token、墙钟和结果重复间方差。

历史复用必须保留生成来源。模型版本、提示、采样参数和检查模板完全相同时，未经结果筛选的历史候选与检查
可以作为同一过程的既有样本；重复读取相同记录不会增加有效样本数。不同模型、提示或采样参数产生的候选属于
未知 off-policy 分布，纯文本接口无法计算 $`p/q`$，因此无法校正回当前基础模型目标。此时可显式把各来源的
抽样比例固定为一个混合生成过程，并把 $`\pi_W`$ 中的基础分布定义为该混合过程；该目标与当前基础模型目标
不同。缓存至少记录模型标识、生成提示哈希、检查提示哈希、采样参数、随机种子、原始候选、原始检查文本和
确定性解析器版本。

本节给出待验证的研究设计，当前仓库尚未实现相应黑盒检查器或报告该算法的实验数值。现有
`sequence_log_probability` 仅用于能够返回精确 token 概率的 AR 后端。
