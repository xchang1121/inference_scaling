...
BF16 诊断使用 `--dtype bfloat16`，输出文件为
`results/validation/rtx3090_backend_bfloat16.json`。

## 后端结果

8 个请求共享 47-token 提示，每个请求生成 24 token。
...
| 单批生成 | 0.352 s |
| 顺序 / 单批墙钟 | 8.02× |
| 单批吞吐 | 545.7 个生成 token/s |
| 共享前缀节省 | 329 个前缀预填充 token |
...
FP32。墙钟数据来自模型加载完成后的重复运行。

请求级随机数序列固定；不同批量形状带来的 CUDA 数值差异，可能使同一个随机阈值落入不同 token 的累积概率
区间。硬件结果同时
记录 token、共同前缀与最终数值。
...
设置：4 条固定长度链，`alpha=2`，总长度 16，生成块长度 8，每个生成块更新 3 次。

| 指标 | 结果 |
| --- | ---: |
| 接受率 | 54.2% |
| 初始平均基础模型对数概率 / token | -1.761 |
| 最终平均基础模型对数概率 / token | -0.971 |
| 差值 | +0.790 |

### 条件 IS

设置：4 条固定的算术题提示，4 个候选，每候选 4 条 rollout，生成块长度 8，二元终局奖励。

| 路径 | 正确数 / 4 | 平均绝对对数修正 | 补全样本的平均有效样本量（ESS） |
| --- | ---: | ---: | ---: |
| Base | 3 | — | — |
...
四个固定基础模型候选各有两条温度 0.7 的历史补全；每个候选追加一条新生成的基础模型补全。
...
决策后的设计集由 8 条已经使用的历史记录和 4 条新生成补全组成；四个历史 ESS 均为 2.0。

### 动态候选 IS

候选 proposal 以 1:1 混合基础模型分布与温度 0.7 的辅助模型分布。六次 2-token 决策中，两种分量各生成
