...
2026 年 8 月 16 日在 NVIDIA GeForce RTX 3090 24 GB 上完成 AR-LLM `smoke` 路线。公开 GSM8K
数据准备、一次真实 GRPO 更新、LoRA 保存与重新加载、11 种推理方法、rollout replay、动态候选、异步
批处理、两组 pass@$`k`$、消融、预算曲线、长度实验、分布诊断和两组 infra 基准均正常结束。训练与推理
总清单的 `status` 均为 `complete`。

该配置用于验证实现、调度、模型加载、概率评分、计算账本和产物恢复。质量部分只取 1 道测试题，
pass@$`k`$ 只取 2 个独立 draw，因此本页不据此比较方法准确率。正式质量结论仍以
...
| NumPy | 1.26.4 |
| 主模型 | Qwen2.5-1.5B-Instruct，revision `989aa7980e4cf806f80c7fef2b1adb7bc71aa306` |
| proposal 模型 | Qwen2.5-0.5B-Instruct |
...
推理入口的续跑清单记录 4 个完成的下层命令。第一次执行发现 adapter 参数被传给 replay 和 dynamic IS
脚本；修正参数作用域后，以同一 tag 续跑，已完成的质量方法直接复用原产物。
...
| 训练样本 | 4 |
| 生成 completion | 8 |
| 生成 completion token | 768 |
| 训练墙钟 | 18.751 s |
| 估计总计算量 | 0.026751 PFLOPs |
| CUDA 峰值 allocated | 3,766,979,584 bytes |
| `nvidia-smi` 峰值显存 | 5,872 MiB |

LoRA adapter 已写入 checkpoint，并由 `rl_sample`、`rl_greedy`、pass@$`k`$ 和分布诊断重新加载。
adapter 权重 SHA-256 为
...
| rollout replay | fresh、cache build、warm online、概率重评分与复现检查 | 候选批次全部复现，rollout 复用率 42.86% |
| 动态 IS | 基础候选、replay-aware 固定分配、方差—成本分配 | 三个分支完成；replay 候选命中率 50% |
| 异步批处理 | Base、Best-of-N、标准 IS、小 proposal IS 的同步/异步成对运行 | 4 组输出逐 token 一致 |
| 通用 pass@$`k`$ | Base、MH、GRPO 随机采样 | 3 种方法、每题 2 个独立 draw 完成 |
| IS pass@$`k`$ | 标准 IS、截断 off-policy、未截断 off-policy、无主模型重评分 | 4 种方法、每题 2 个独立 draw 完成 |
| 消融 | MH 轮数与锐化系数、候选数、rollout 数、guidance steps、奖励来源、温度 | 所有 `smoke` 分支完成 |
| 预算与长度 | Beam、Best-of-N、两类 IS 的预算点；32-token 短输出路径 | 完成 |
| 分布诊断 | Base、verifier-MH、两类 verifier-IS、GRPO，共 10 次生成 | 完成并记录模型、adapter 和实现哈希 |
| rollout infra | 历史树、负载感知草稿、progressive IS、run-ahead、SMC | 3 个解码分支和 5 个算法分支完成 |
| IS/MH infra | partial rollout、流式 IS、随机草稿、MH 预取、延迟接受、历史 proposal | 17 个执行分支完成 |
...
| 检查 | 观测值 |
| --- | ---: |
| replay fresh / warm online 墙钟比 | 1.0167 |
| replay fresh / warm online FLOPs 比 | 1.1157 |
| replay base / proposal 概率缓存命中率 | 50% / 50% |
| 流式 IS 首次估计更新时间：整批等待 | 3.036 s |
| 流式 IS 首次估计更新时间：完成即提交 | 0.820 s |
| 带 0.2 s verifier 延迟时的首次更新时间 | 3.324 s → 1.008 s |
| MH proposal-tree 预取与普通 MH 的路径校验 | 两组设置均一致 |
| delayed-acceptance 精确奖励调用 | 9 → 8，最终 token hash 一致 |
| 历史混合 proposal 与基础 proposal 的平均最终奖励 | 0.75 / 0.75 |
| 历史混合 proposal 与基础 proposal 的墙钟 | 9.535 s / 35.380 s |

单次短基准包含调度抖动，表中墙钟只用于确认机制和计量路径。稳定的加速结论见三 seed 的
...
| 问题 | 修正 | 验证 |
| --- | --- | --- |
| NumPy 版本下界与既有科学计算栈冲突 | 支持 NumPy 1.26 及以上 | `pip check` 通过；AR 测试解释器可导入全部训练依赖 |
| replay / dynamic IS 收到无效的 `--rl-adapter` | adapter 只传给质量、pass@$`k`$ 和分布脚本 | 参数路由回归测试通过；统一入口续跑完成 |
| 通用 pass@$`k`$ 未写入 `summary-root` | 显式传入聚合输出路径 | 修复后重新执行 3 种方法，聚合文件位于本次验证目录 |
| 嵌套验证目录中的 raw records/chunks 可能进入 Git | 使用递归忽略规则 | Git 只收录聚合 JSON 与 suite manifest |
...
logprob 解析、分组和错误处理由单元测试与 mock engine 覆盖；本次真实模型运行使用 Transformers。
真实 vLLM engine 验证需要 Linux 或 WSL2 CUDA 环境，入口和参数保持一致。
