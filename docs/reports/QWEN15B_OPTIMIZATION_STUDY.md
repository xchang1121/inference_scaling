...
index。仓库已实现解析区间判定、分批 rollout 与越界检查。筛选使用冻结 pilot 二值奖励，在
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| fresh-only，顺序 | 0 | 8.336 | 0.016022 | 0 | 0.016022 | 8.336 |
| fresh-only，连续批处理 | 0 | 4.131 | 0.017205 | 0 | 0.017205 | 4.131 |
| warm replay，顺序 | 5.905 | 8.351 | 0.014490 | 0.002562 | 0.017052 | 14.255 |
| warm replay，候选缓存 | 5.860 | 5.817 | 0.011696 | 0.002562 | 0.014258 | 11.677 |
| warm replay，候选缓存与连续批处理 | 2.498 | 3.133 | 0.012794 | 0.002801 | 0.015596 | 5.631 |

候选缓存相对顺序 warm replay 的在线墙钟因子为 `0.697 ± 0.009×`，1.5B FLOPs 因子为
`0.807 ± 0.004×`。在候选缓存上加入连续批处理，墙钟因子为 `0.537 ± 0.092×`；padding 使该局部
对比的 1.5B FLOPs 和总 FLOPs 均为 `1.094×`。完整在线栈相对已经启用连续批处理的
fresh-only 路径，墙钟因子为 `0.754 ± 0.093×`，1.5B FLOPs 因子为 `0.744×`，合计 FLOPs 因子为
`0.907×`。fresh 顺序/批处理、replay 候选缓存前后、replay 顺序/批处理三类配对在三个 seed 中均逐
token 一致。平均 rollout 复用率为 31.62%。

若当前请求还需生成并评分 history，完整冷启动相对连续批处理 fresh-only 的墙钟和总 FLOPs 分别为
`1.361×` 和 `1.941×`。默认路径只在存在匹配且尚未消费的 evaluation 记录时启用 warm replay；否则使用
...
| IS replay + 候选缓存 + 连续批处理 | 正收益 | 有匹配且未消费的 history 时启用 | 相对连续批处理 fresh-only，在线墙钟 `0.754×`、1.5B FLOPs `0.744×`、总 FLOPs `0.907×` |
